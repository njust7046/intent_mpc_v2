...
namespace controller{
	...
	void trackingController::publishCommand(const Eigen::Vector3d& accRef){
		...
		cmdMsg.header.stamp = ros::Time::now();
		cmdMsg.header.frame_id = "map";
		if (std::isnan(accRef(0)) || std::isnan(accRef(1)) || std::isnan(accRef(2))){
			cmdMsg.acceleration_or_force.x = 0.0;
			cmdMsg.acceleration_or_force.y = 0.0;
			cmdMsg.acceleration_or_force.z = 0.0;
		} else {
