...
void DroneSimpleController::UpdateDynamics(double dt){
  ...
  //  if (imu_subscriber_.getTopic()=="")
   ...
   // if (state_topic_.empty())
    ...
    // Sanitize NaN values to prevent Gazebo model reset
    if (isnan(force.X()) || isnan(force.Y()) || isnan(force.Z())) force.Set(0.0, 0.0, 0.0);
    if (isnan(torque.X()) || isnan(torque.Y()) || isnan(torque.Z())) torque.Set(0.0, 0.0, 0.0);
