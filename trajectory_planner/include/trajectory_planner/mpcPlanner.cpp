...
bool mpcPlanner::solveTraj(const std::vector<staticObstacle> &staticObstacles, const std::vector<std::vector<Eigen::Vector3d>> &dynamicObstaclesPos, const std::vector<std::vector<Eigen::Vector3d>> &dynamicObstaclesSize, 
	...
	// Check for NaN in QP inputs
	if (gradient.hasNaN() || lowerBound.hasNaN() || upperBound.hasNaN()){
		std::cout << "[MPCPlanner]: NaN in QP inputs -"
			<< " gradient:" << gradient.hasNaN()
			<< " lower:" << lowerBound.hasNaN()
			<< " upper:" << upperBound.hasNaN()
			<< " x0:[" << x0(0) << "," << x0(1) << "," << x0(2) << "]"
			<< " numObs:" << numObs
			<< " firstTime:" << this->firstTime_
			<< std::endl;
		this->currentStatesSol_.clear();
		this->currentControlsSol_.clear();
		this->firstTime_ = true;
	...
	// Check for NaN in solution - if found, reset state to force fresh start next time
	if (QPSolution.hasNaN()){
		std::cout << "[MPCPlanner]: QP solution contains NaN. Resetting solver state." << std::endl;
		this->currentStatesSol_.clear();
		this->currentControlsSol_.clear();
		this->firstTime_ = true;
		return 0;
	...
	void mpcPlanner::castMPCToQPConstraintMatrix(Eigen::Matrix<double, numStates, numStates> &A, Eigen::Matrix<double, numStates, numControls> &B, 
		...
		for (int i = 0; i < mpcWindow; i++){
			...
			for (int j = 0; j < numObs; j++){
				double fxx,fyy,fzz;
				double sx = std::max(osize[i](j,0), 0.01);
				double sy = std::max(osize[i](j,1), 0.01);
				double sz = std::max(osize[i](j,2), 0.01);
				...
				fxx = 2*((cx-oxyz[i](j,0))*cos(yaw[i](j,0))+(cy-oxyz[i](j,1))*sin(yaw[i](j,0)))/pow(sx,2)*cos(yaw[i](j,0))+ 2*(-(cx-oxyz[i](j,0))*sin(yaw[i](j,0))+(cy-oxyz[i](j,1))*cos(yaw[i](j,0)))/pow(sy,2)*(-sin(yaw[i](j,0)));
				fyy = 2*((cx-oxyz[i](j,0))*cos(yaw[i](j,0))+(cy-oxyz[i](j,1))*sin(yaw[i](j,0)))/pow(sx,2)*sin(yaw[i](j,0))+ 2*(-(cx-oxyz[i](j,0))*sin(yaw[i](j,0))+(cy-oxyz[i](j,1))*cos(yaw[i](j,0)))/pow(sy,2)*(cos(yaw[i](j,0)));
				fzz = 2*((cz-oxyz[i](j,2)))/pow(sz,2);
	...
	void mpcPlanner::castMPCToQPConstraintVectors(Eigen::Matrix<double,numStates,1> &xMax, Eigen::Matrix<double,numStates,1> &xMin,
		...
		for (int i = 0; i < mpcWindow; i++){
			...
			for (int j = 0; j < numObs; j++){
				...
				double sy = std::max(osize[i](j,1), 0.01);
				double sz = std::max(osize[i](j,2), 0.01);
				fxyz = pow((cx-oxyz[i](j,0))*cos(yaw[i](j,0))+(cy-oxyz[i](j,1))*sin(yaw[i](j,0)), 2)/pow(sx,2) + pow(-(cx-oxyz[i](j,0))*sin(yaw[i](j,0))+(cy-oxyz[i](j,1))*cos(yaw[i](j,0)), 2)/pow(sy,2) + pow((cz-oxyz[i](j,2)), 2)/pow(sz,2);
				fxx = 2*((cx-oxyz[i](j,0))*cos(yaw[i](j,0))+(cy-oxyz[i](j,1))*sin(yaw[i](j,0)))/pow(sx,2)*cos(yaw[i](j,0))+ 2*(-(cx-oxyz[i](j,0))*sin(yaw[i](j,0))+(cy-oxyz[i](j,1))*cos(yaw[i](j,0)))/pow(sy,2)*(-sin(yaw[i](j,0)));
				fyy = 2*((cx-oxyz[i](j,0))*cos(yaw[i](j,0))+(cy-oxyz[i](j,1))*sin(yaw[i](j,0)))/pow(sx,2)*sin(yaw[i](j,0))+ 2*(-(cx-oxyz[i](j,0))*sin(yaw[i](j,0))+(cy-oxyz[i](j,1))*cos(yaw[i](j,0)))/pow(sy,2)*(cos(yaw[i](j,0)));
				fzz = 2*((cz-oxyz[i](j,2)))/pow(sz,2);
