...
namespace trajPlanner{
	...
	void bsplineTraj::assignGuidePointsSemiCircle(const std::vector<std::vector<Eigen::Vector3d>>& paths, const std::vector<std::pair<int, int>>& collisionSeg){
		...
		for (size_t i=0; i<collisionSeg.size(); ++i){
			...
			for (int controlPointIdx=seg.first+1; controlPointIdx<seg.second; ++controlPointIdx){ // iterate through all collision control points
				...
				Eigen::Vector3d guideDiff = guidePoint - this->optData_.controlPoints.col(controlPointIdx);
				double guideDiffNorm = guideDiff.norm();
				if (guideDiffNorm < 1e-6){
					guideDirection = Eigen::Vector3d::Zero();
				} else {
					guideDirection = guideDiff / guideDiffNorm;
				}
			...
			if (lineCollision){
				...
				Eigen::Vector3d midDiff = guidePoint - midPoint;
				double midDiffNorm = midDiff.norm();
				if (midDiffNorm < 1e-6){
					guideDirection = Eigen::Vector3d::Zero();
				} else {
					guideDirection = midDiff / midDiffNorm;
				}
	...
	double bsplineTraj::costFunction(const double* x, double* grad, const int n){
		int expectedN = 3 * (this->optData_.controlPoints.cols() - 2*bsplineDegree);
		if (n != expectedN || this->optData_.controlPoints.cols() == 0){
			memset(grad, 0, n*sizeof(grad[0]));
			return 0.0;
		}
		if ((int)this->optData_.guidePoints.size() != (int)this->optData_.controlPoints.cols()){
			memset(grad, 0, n*sizeof(grad[0]));
			return 0.0;
		}
		...
		for (int i=bsplineDegree; i<=controlPoints.cols()-bsplineDegree-1; ++i){
			...
			for (size_t j=0; j<this->optData_.dynamicObstaclesPos.size(); ++j){
				...
				for (int n=0; n<=predictionNum; n+=skipFactor){
					...
					if (diffNorm < 1e-6) continue; // avoid division by zero
					double dist = diffNorm - size; // actual distance to obstacle (need to minus obstacle size)
					double distErr = distThresh - dist;
					Eigen::Vector3d grad = diff/diffNorm;
