	...
	void mpcNavigation::mpcCB(){
		...
		while (ros::ok()){
			if (this->mpcReplan_){
				...
				std::cout << "[MPC-DBG] currPos=" << this->currPos_.transpose() << " odom=" << this->odom_.pose.pose.position.x << "," << this->odom_.pose.pose.position.y << "," << this->odom_.pose.pose.position.z << std::endl;
				if (not this->refTrajReady_){
					...
					if (currPos.norm() < 1e-6 && currVel.norm() < 1e-6){
						r.sleep();
						continue;
					}
